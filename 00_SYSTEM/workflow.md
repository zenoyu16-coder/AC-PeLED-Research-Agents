...
| source verification | paper card、DOI、publisher 或 authoritative metadata | source status 与 claim evidence classification | DOI metadata 一致；architecture scope 清楚；unsupported mechanism claim 被阻断 |
...
进入 mechanism mapping 前必须完成：

- 解析 DOI，并核对 title、authors、journal 与 year。
- 将每个关键 claim 分类为 `direct evidence`、`indirect support`、`author hypothesis` 或 `background context`。
- 检查 architecture scope，PeLEFET、OLED、PeLED 与 material-only 研究不得视为等价证据。
- 每个 claim 必须有 evidence locator；abstract-only evidence 不得冒充 figure-level evidence。
- unsupported 或 architecture-mismatched mechanism claim 标为 `UNVERIFIED` 并阻断其进入机制链。

## Gate 3: Experiment Readiness
...
- source verification 完成，DOI metadata、architecture scope 与 verification status 可追溯。
